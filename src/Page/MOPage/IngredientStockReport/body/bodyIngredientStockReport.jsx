...
import IngredientStockTable from "../component/ingredientStockReportTable";
import LoadingTable from "../component/LoadingTable";
import { PDFDownloadLink, pdf } from "@react-pdf/renderer";
import PDFIngredientStock from "../component/PDFIngredientStock";
import { GetAllIngredients } from "../../../../api/IngredientApi";

export default function BodyIngredientStockReport({ search }) {
  const date = new Date();
  const [month, setMonth] = useState(
    date.toLocaleString("default", { month: "long" }),
  );

  const ingredientReport = useQuery({
    queryKey: ["ingredientReport"],
  ...
  return (
    ...
    >
      ...
      <div>
        {ingredientReport.isFetching ? (
          <LoadingTable loading={ingredientReport.isFetching} />
        ) : (
          <div>
            {console.log("data", ingredientReport.data)}
            <div className="pt-6">
              <IngredientStockTable
                search={search}
                data={ingredientReport.data}
                length={ingredientReport.data.length}
              />
            </div>
          </div>
        )}
